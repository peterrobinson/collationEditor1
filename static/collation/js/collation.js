...
//			word = '%s%s' % (word, token['pc_after'])
		...
		get_json_apparatus: function (callback) {
			var collation;
			CL._services.get_user_info(function (user) {
				if (user) { //we only do this for approved collations
					...
					CL._services.get_json_apparatus(collation, callback);
				}
			})
			callback();
		},
		...
		_add_shiftkey_events: function (row, witnesses) {
...
//					}
		...
		load_single_project_menu: function (project, application, callback) {
